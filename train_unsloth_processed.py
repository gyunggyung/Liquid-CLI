...
def main(args):
    ...
    # 2. 데이터 로딩 및 포맷팅 (캐시 로직 추가)
    processed_data_path = args.data_path + "_unsloth_processed"
    
    if os.path.exists(processed_data_path):
        print(f"\n[2/4] 🚀 가공된 데이터셋 발견! 전처리를 건너뛰고 즉시 로드합니다: {processed_data_path}")
        dataset = load_from_disk(processed_data_path)
    else:
        print(f"\n[2/4] 데이터 로딩 및 포맷팅 중... (최초 1회만 수행): {args.data_path}")
        ...
        print(f"💾 가공된 데이터셋 저장 중: {processed_data_path}")
